...
def start():
    ...
    canvas = scene.SceneCanvas(keys='interactive',fullscreen=True, show=True, bgcolor='black')  # Removed fullscreen for controls
    canvas.show()
...
def smooth_zoom(event):
    ...
    if current_step[0] > steps:
        ...
        # Animation is complete, move to next stage
        is_zooming = False
          # Signal for node meeting animation

def start_zoom(event=None):
    global target_xmin, target_xmax, target_ymin, target_ymax, timer, is_zooming
    
    # Don't start if already zooming
    if is_zooming:
        return
...
def start_node_meeting_animation(event=None):
    ...
    # Don't start if already animating nodes
    if is_node_meeting:
        return
    ...
    # Skip if no nodes to move
    if len(move_indices) == 0:
        is_node_meeting = False
        simulation_stage = 2  # Skip to death check
        return
...
def calculate_move_positions():
    global positions,infected,infected_count,temp_infected, scatter, move_indices
    ...
    for i in range(5000):
        if to_process[i].empty() or not is_alive[i]:
            ...
            if to_process[i].empty():
                for node in adj[x[0]]:
                    if is_alive[node[0]]:  # Only process alive nodes
            ...
            # Only add pairs where both nodes are alive
    ...
    # Only proceed if there are nodes to move
    if move_indices:
...
def death_check(event=None):
    global is_alive, infected_count, positions, colors, scatter, sizes, to_zoom
    global is_death_checking, simulation_stage
...
def move_nodes_meeting(event):
    ...
    global move_phase, infected_count, is_node_meeting, simulation_stage
    ...
    if move_current_step[0] <= steps:
        ...
        # Skip if index arrays are empty
        if len(idx1) == 0 or len(idx2) == 0:
            move_timer.stop()
            is_node_meeting = False
            simulation_stage = 2  # Move to death check
            return
...
def scatter_graph_on_midpoints():
    ...
    # Only proceed if midpoints exist
...
def blink_and_remove_new_scatter(callback=None):
    ...
    # Don't start if already blinking or no scatter to blink
    ...
    def blink(event):
        ...
        if new_scatter is not None:  # Check if scatter still exists
        ...
        if step[0] >= total_steps:
            ...
            blink_timer.stop()
            is_blinking = False  # Mark blinking as complete
            if callback:
                callback()
...
def main_loop_step(event):
    ...
    # Don't start a new animation if any current one is running
    if is_zooming or is_node_meeting or is_death_checking or is_blinking:
        return
    
    # State machine approach - check the current stage and start appropriate animation
...
# Initialize the main loop timer
main_loop_timer = None
simulation_stage = -1  # Starting stage
interval = 0.5  # Check state every 0.5 seconds

if __name__ == '__main__':
    load_data()
    start()
    # Start the main loop timer - this will check if we need to start a new animation stage
    main_loop_timer = Timer(interval=interval, connect=main_loop_step, start=True)
    app.run()